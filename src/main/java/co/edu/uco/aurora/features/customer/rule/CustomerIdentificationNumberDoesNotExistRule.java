...
public final class CustomerIdentificationNumberDoesNotExistRule implements Rule {
    ...
    public void execute(Object... data) {
        ...
        if (ObjectHelper.isNull(data)) {
            var userMessage = MessagesEnum.CUSTOMER_ID_NUMBER_DOES_NOT_EXIST_RULE_DATA_IS_NULL.name();
            throw AuroraException.create(userMessage);
        }

        if (data.length < 2) {
            var userMessage = MessagesEnum.CUSTOMER_ID_NUMBER_DOES_NOT_EXIST_RULE_DATA_LENGTH_INVALID.name();
            throw AuroraException.create(userMessage);
        }
        ...
        if (exists) {
            var userMessage = MessagesEnum.CUSTOMER_ID_NUMBER_DOES_NOT_EXIST_RULE_CUSTOMER_ALREADY_EXISTS.name();
            throw AuroraException.create(userMessage);
        }
